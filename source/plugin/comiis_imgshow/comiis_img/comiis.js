...
function errorhandle_kmtwt(msg, arg){
	if(msg){
		showDialog(msg, 'notice', '错误', null, true, null, '', '', '', 3);
	}
}
function succeedhandle_kmtwt(url, msg, values) {
	showDialog('回复发布成功', 'right', '提示', function () { location.href = comiis_reurl; }, true, null, null, null, null, null, 3);
}
function comiis_post(){
	if($('fastpostmessage').value == '文明上网，理性发言' || $('fastpostmessage').value == ''){
		showDialog('请输入要发表的内容!', 'right', '提示');
	}else{
		if($("image_message").checked){
			$('fastpostmessage').value= tname + "\n\r[img]" + hdPic.fn._tmpArray[hdPic.fn._pageNow].bigpic + "[/img]\n\r" + $('fastpostmessage').value;
		}
		ajaxpost('fastpostform', 'fastpostreturn');
	}
}
...
var loadingProcess = { //全屏播放用
    ...
    isSwfReady: false,
    divName: 'fullSwf',
    swfUrl: './source/plugin/comiis_imgshow/comiis_img/comiis_FullScreen.swf',
    sitePicUrl: '#',
    lastUrl: '#',
    datas: null,
    flashNub: 0,
...
hdPic.fn = hdPic.prototype = {
    ...
    _getReady: function() { //大图首次载入ready，初始化播放器区域高度，图片切换效果、hover效果
        jQuerys(".pageLeft-bg").show();
        jQuerys(".pageRight-bg").show();
        jQuerys(".pageLeft").height(jQuerys("#Main-A").height());
        jQuerys(".pageLeft span").css("marginTop", parseInt((jQuerys("#Main-A").height() - 95) / 2));
        jQuerys(".pageLeft-bg").height(jQuerys("#Main-A").height());
        jQuerys(".pageRight").height(jQuerys("#Main-A").height());
        jQuerys(".pageRight span").css("marginTop", parseInt((jQuerys("#Main-A").height() - 95) / 2));
        jQuerys(".pageRight-bg").height(jQuerys("#Main-A").height());
        jQuerys("#mouseOverleft").height(jQuerys("#Main-A").height());
        jQuerys("#mouseOverleft").width(parseInt(jQuerys("#Main-A").width() / 2));
        jQuerys("#mouseOverright").height(jQuerys("#Main-A").height());
        jQuerys("#mouseOverright").width(parseInt(jQuerys("#Main-A").width() / 2));
        jQuerys("#mouseOverleft").hover(function() {
            jQuerys(".pageLeft").fadeIn("fast");
            jQuerys(".pageLeft").attr("title", "点击浏览上一张图片，支持'←'翻页");
        }, function() {
            jQuerys(".pageLeft").fadeOut("fast");
            jQuerys(".pageLeft").attr("title", "");
        });
        jQuerys("#mouseOverright").hover(function() {
            jQuerys(".pageRight").fadeIn("fast");
            jQuerys(".pageRight").attr("title", "点击浏览下一张图片，支持'→'翻页");
        }, function() {
            jQuerys(".pageRight").fadeOut("fast");
            jQuerys(".pageRight").attr("title", "");
        });
        if (jQuerys("#Main-D").css("display") == "block") {
            jQuerys("#Main-D").fadeTo('slow', 1).fadeTo('slow', 0.2).fadeTo('slow', 1);
        }
        jQuerys("#Main-C").fadeTo('slow', 1).fadeTo('slow', 0.2).fadeTo('slow', 1);
    },
    ...
    _clickright: function(data) { //向后点
        if (hdPic.fn._pageNow < data.length - 1) {
            ...
            var left1 = 131 * (hdPic.fn._tmpArray.length - 7), left2 = parseInt(jQuerys("#Smailllist").css('left'));
    ...
    _bindClick: function(data) { //为各种按钮绑定事件、拖拽浏览、快捷键、页面初始焦点
        var _this = this;
        jQuerys("#Smailllist li").each(function(i) {
            jQuerys(this).click(function() {
                hdPic.fn._stopAuto();
        ...
        jQuerys("#mouseOverright").bind('click', function() {
            _calTime();
            hdPic.fn._stopAuto();
            hdPic.fn._clickright(data);
        });
        jQuerys("#goright").bind('click', function() {
            _calTime();
            hdPic.fn._stopAuto();
            hdPic.fn._clickright(data);
        });
        jQuerys("#mouseOverleft").bind('click', function() {
            _calTime();
            hdPic.fn._stopAuto();
            hdPic.fn._clickleft(data);
        });
        jQuerys("#goleft").bind('click', function() {
            _calTime();
            hdPic.fn._stopAuto();
            hdPic.fn._clickleft(data);
        });
        //拖拽浏览
        if (hdPic.fn._tmpArray.length > 7) {
            jQuerys(".scrollButton").bind("selectstart", function() {
                return false;
            });
            jQuerys(".scrollButton").click(function() {}).mousedown(function(e) {
                _this._isDoc = true;
                hdPic.fn._stopAuto();
                //设置捕获范围
                if (jQuerys(".scrollButton").setCapture) {
                    jQuerys(".scrollButton").setCapture();
                } else if (window.captureEvents) {
                    window.captureEvents(Event.MOUSEMOVE | Event.MOUSEUP);
                }
                hdPic.fn._isMove = true;
                hdPic.fn._dragx = e.pageX - parseInt(jQuerys(".scrollButton").css("left"));
                jQuerys(".scrollButton").fadeTo(20, 0.5);
                jQuerys("a.mask").hide();
            });
            jQuerys(document).mousemove(function(e) {
                if (hdPic.fn._isMove) {
                    var x = Math.max(0, Math.min(e.pageX - hdPic.fn._dragx, 759));
                    jQuerys(".scrollButton").css({
                        left: x
                    });
                    hdPic.fn._dragmov();
                }
                ...
                //取消捕获范围
                if (jQuerys(".scrollButton").releaseCapture) {
                    jQuerys(".scrollButton").releaseCapture();
                } else if (window.captureEvents) {
                    window.captureEvents(Event.MOUSEMOVE | Event.MOUSEUP);
                }
                jQuerys(".scrollButton").fadeTo("fast", 1);
                if (parseInt(jQuerys("#Smailllist").css("left")) % 131 !== 0 && _this._isDoc) {
                    var argleft = parseInt(jQuerys("#Smailllist").css("left"));
                    jQuerys("#Smailllist").animate({
                        left: argleft + (Math.abs(parseInt(jQuerys("#Smailllist").css("left")) % 131)) + "px"
                    },"fast");
                };
                _this._isDoc = false;
            })
        ...
        //快捷键
        jQuerys(document).bind("keydown", function(e) {
            e = window.event || e;
            hdPic.fn._stopAuto();
    ...
    _stopAuto: function() { //停止自动播放
        jQuerys(".play").html("幻灯播放");
        jQuerys(".play").removeClass("stop");
        hdPic.fn._isAuto = false;
        window.clearInterval(hdPic.fn._autoTimer);
    },
    _autoplay: function(data) { //自动播放
        jQuerys(".play").html("停止播放");
        jQuerys(".play").addClass("stop");
        ...
        this._autoTimer = window.setInterval(function() {
            if (hdPic.fn._pageNow < data.length - 1) {
                ...
                hdPic.fn._stopAuto();
                hdPic.fn._showLast(data);///////////////////////////////////////////////////
            }
        }, 5000)
    },
    _dragmov: function() { //拖拽浏览用函数
        var a = parseInt(131 * (parseInt(jQuerys(".scrollButton").css("left")) / 759) * (this._tmpArray.length - 7));
        jQuerys("#Smailllist").css({
            left: -a + "px"
        });
    },
    ...
    _showtit: function(n) { //设置图片图注显示
        ...
        jQuerys("#Main-B").html('<P>' + this._tmpArray[n].showtit + '</p>'+kmtitle); //图注装载
        jQuerys("#Main-B").fadeIn("slow");
        loadingProcess.setPicHandler(n);
    },
    _creatUrl: function(n) { //创建组图浏览url标识
        var _org = /\#p\=/i.test(window.location.href);
        if (!_org) {
			reurl=window.location.href + comiis_Rewrite + parseInt(10*Math.random()) + "#p=1";
            window.location.href = window.location.href + "#p=1";
        } else {
			reurl=window.location.href.split("#p=")[0];
			reurl=reurl.split(comiis_Rewrite)[0] + comiis_Rewrite + parseInt(10*Math.random()) + "#p=" + parseInt(n + 1);
    ...
    _showBig: function(data, n) { //显示大图、显示成功后设置索引值对应的图注、url、组图当前索引值改写、小图位置、统计
        indexPic = n;
        jQuerys("#orgPic").attr("href", data[n].bigpic);
		...
		jQuerys("#PicSrc").load(function() {
			img.src = data[n].bigpic;
			var w = img.width;
			if (w > 980) w = 980;
			jQuerys("#PicSrc").css('width', w);
			jQuerys("#Main-A").height(jQuerys("#PicSrc").height());
			jQuerys(".pageLeft").height(jQuerys("#Main-A").height());
			jQuerys(".pageLeft span").css("marginTop", parseInt((jQuerys("#Main-A").height() - 95) / 2));
			jQuerys(".pageLeft-bg").height(jQuerys("#Main-A").height());
			jQuerys(".pageRight").height(jQuerys("#Main-A").height());
        ...
        jQuerys("#PicSrc").fadeTo("fast", 0, function() {
            hdPic.fn._showSmall(n);
            hdPic.fn._showtit(n);
            hdPic.fn._creatUrl(n);
            jQuerys("#PicSrc").attr("src", data[n].bigpic);
            jQuerys("#PicSrc").fadeTo("fast", 1);
        });
        hdPic.fn._pageNow = n;
    },
    _showSmall: function(n) { //小图移动切换逻辑
        ...
        jQuerys("a.mask").show();
        if (_len <= 7) {
            jQuerys("a.mask").animate({
                left: (131 * n) + 4 + "px"
            }, "slow");
            return false;
        }
        var _left, _latsindex = 7;
        if (n >= 3 && n < _len - 3) { //大于3小于倒数3
            setTimeout(function() {
                if (_this._clickSum > 1) {
                    ...
                    jQuerys("#Smailllist").stop(true, true).animate({
                        left: -131 * (n - 3) + "px"
                    }, "slow");
                }
            }, 200);
    ...
    _getData: function(data) { //第一次加载后，初始化大图、小图、绑定事件、统计等
        if (data.length > 0) {
            /*成功*/
            jQuerys("#Main-A").append("<img src=" + data[parseInt(hdPic.fn._getUrl() - 1)].bigpic + " id='PicSrc' style='display:none'/>");
            this._getReady(); //大图ready
            this._small(data); //装载小图
            this._pageNow = parseInt(hdPic.fn._getUrl() - 1);
            this._bindClick(data);
            jQuerys("#orgPic").attr("href", data[parseInt(hdPic.fn._getUrl() - 1)].bigpic);
    ...
    _small: function(data) { //第一次加载后初始化小图
        ...
        jQuerys("#Smailllist").width(ulLength);
        jQuerys("#Smailllist").html(_tmp);
    },
	...
	_getLast:function(data){//末页推荐
		...
		setTimeout(function(){//末页推荐延迟100ms
			hdPic.fn._showLast(data,siteN);
		},100);
	},
	_showLast:function(data,siteN){
		this.AD = true;
		jQuerys("#end").css("left",parseInt((jQuerys("#Main-A").width()/2-jQuerys("#end").width()/2))+"px");
		jQuerys("#Main-A").height(jQuerys("#Main-A").height());
		jQuerys("#end").animate({top:"130px"},"slow",function(){
					jQuerys(".pageLeft").height(jQuerys("#Main-A").height());
					jQuerys(".pageLeft span").css("marginTop",parseInt((jQuerys("#Main-A").height()-95)/2));
					jQuerys(".pageLeft-bg").height(jQuerys("#Main-A").height());
					jQuerys(".pageRight").height(jQuerys("#Main-A").height());
	  ...
	  jQuerys("#replayPic").bind("click",function(){
			hdPic.fn._hideLast();
			hdPic.fn._pageNow = 0;
			hdPic.fn._showBig(data,hdPic.fn._pageNow);
	  });
	  jQuerys("a.close").bind("click",function(){
			hdPic.fn._hideLast();
			hdPic.fn._showBig(data,hdPic.fn._pageNow);
	  });
	},
	_hideLast:function(){//隐藏末页推荐
		...
		jQuerys("#end").animate({top:"-1000px"},"slow",function(){
				jQuerys(".pageLeft").height(jQuerys("#Main-A").height());
				jQuerys(".pageLeft span").css("marginTop",parseInt((jQuerys("#Main-A").height()-95)/2));
				jQuerys(".pageLeft-bg").height(jQuerys("#Main-A").height());
				jQuerys(".pageRight").height(jQuerys("#Main-A").height());
...
jQuerys(document).ready(function(jQuerys) {
	hdPic.fn._query();
});